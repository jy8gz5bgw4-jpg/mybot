...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "telegram":
        ...
        if row and row[0]:
            cur.execute("SELECT user_id, channel_link FROM users WHERE channel_link IS NOT NULL AND user_id != ?", (user_id,))
            others = cur.fetchall()
            if others:
                cur.execute("SELECT to_user FROM actions WHERE from_user = ? AND done = 1", (user_id,))
                ...
                await query.edit_message_text("Пока нет других каналов. Вставь свою ссылку.")
        else:
            await query.edit_message_text("Вставь ссылку на свой Telegram-канал (t.me/... или t.me/joinchat/...)")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("waiting_link"):
        conn = sqlite3.connect("subscriptions.db")
        cur = conn.cursor()
        cur.execute("INSERT OR REPLACE INTO users (user_id, channel_link) VALUES (?, ?)", (user_id, text))
        conn.commit()
        conn.close()
...
async def wait_and_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect("subscriptions.db")
    cur = conn.cursor()
    cur.execute("INSERT INTO actions (from_user, to_user, done) VALUES (?, ?, 1)", (user_id, target_id))
    cur.execute("UPDATE users SET subscribed_count = subscribed_count + 1 WHERE user_id = ?", (user_id,))
    conn.commit()
    conn.close()
